...
import { Cambio, Plate } from '../../../_bootstrap/models/models'
...
import { DaoCambiosService } from 'src/app/admin/services/dao-cambios.service'
...
import { DaoBarService } from 'src/app/waiter/services/dao-bar.service'
...
export class MenuFormComponent implements OnInit {
  ...
  constructor(private dao: DaoRecipeService, private daoPlate: DaoPlateService, private daoEmployeeService: DaoEmployeeService, private daoBar: DaoBarService, private messageService: MessageService, private router: Router, private daoCambios: DaoCambiosService) {}
  ...
  initEmployees() {
    this.daoEmployeeService.getEmployees().subscribe((res) => {
      console.log(res)
      this.adminList = res.filter((res) => res.role == BUSINESS_ROLE.ADMIN)
      this.chefList = res.filter((res) => res.role == BUSINESS_ROLE.CHEF)
      this.waiterList = res.filter((res) => res.role == BUSINESS_ROLE.WAITER)
      this.deliveryList = res.filter((res) => res.role == BUSINESS_ROLE.DELIVERY)
    })
  }
