...
import { PlateOrder } from '../../../bootstrap/models/models'
...
export class DaoPlateOrderService {
  ...
  getOpenOrdersNextPage(pageSize, item) {
    let col = this.afs.collection(this.fsCollectionName, (ref) =>
      ref.where('isDelivered', '==', false).where('isPaid', '==', false).orderBy('timestamp', 'desc').startAfter(item['timestamp']).limit(pageSize)
    )
    return col.snapshotChanges().pipe(
      map((res: any) => {
        return res.map((res) => {
  ...
  getOpenOrdersPreviousPage(pageSize, item) {
    let col = this.afs.collection(this.fsCollectionName, (ref) =>
      ref.where('isDelivered', '==', false).where('isPaid', '==', false).orderBy('timestamp', 'desc').endBefore(item['timestamp']).limitToLast(pageSize)
    )
    return col.snapshotChanges().pipe(
      map((res: any) => {
        return res.map((res) => {
  ...
  getPaidOrdersNextPage(pageSize, item) {
    let col = this.afs.collection(this.fsCollectionName, (ref) =>
      ref.where('isDelivered', '==', true).where('isPaid', '==', true).orderBy('timestamp', 'desc').startAfter(item['timestamp']).limit(pageSize)
    )
    return col.snapshotChanges().pipe(
      map((res: any) => {
        return res.map((res) => {
  ...
  getPaidOrdersPreviousPage(pageSize, item) {
    let col = this.afs.collection(this.fsCollectionName, (ref) =>
      ref.where('isDelivered', '==', true).where('isPaid', '==', true).orderBy('timestamp', 'desc').endBefore(item['timestamp']).limitToLast(pageSize)
    )
    return col.snapshotChanges().pipe(
      map((res: any) => {
        return res.map((res) => {
