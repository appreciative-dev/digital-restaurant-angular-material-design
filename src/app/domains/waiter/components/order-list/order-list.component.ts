...
export class OrderListComponent implements OnInit {
  ...
  constructor(private daoOrder: DaoPlateOrderService, private router: Router) {}
  ...
  loadOrdersCancelado() {
    ...
    this.daoOrder.getPaidOrdersFirstPage(this.PAGE_ITEMS_SIZE).subscribe((res) => {
  ...
  nextCocinado() {
    this.daoOrder.getPaidOrdersNextPage(this.PAGE_ITEMS_SIZE, this.lastItem).subscribe((res) => {
      this.datasourcePaid = new MatTableDataSource(res)
  ...
  previousCocinado() {
    this.daoOrder.getPaidOrdersPreviousPage(this.PAGE_ITEMS_SIZE, this.firstItem).subscribe((res) => {
      this.datasourcePaid = new MatTableDataSource(res)
      this.firstItem = [...res][0]
      this.lastItem = [...res].pop()
    })
